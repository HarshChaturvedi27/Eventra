...
// --- UPDATED: List of major Indian cities ---
...
export default function Navbar() {
  ...
  // --- UPDATED: State to hold selected city, default to Mumbai ---
  ...
  if (!isMounted) {
    // Render a basic static navbar during SSR to prevent hydration errors
    return (
       <nav className="fixed top-0 left-0 right-0 z-50 bg-white shadow-md">
         <div className="container mx-auto px-6 flex justify-between items-center h-20">
             ...
             {/* Simplified static elements for SSR */}
         </div>
       </nav>
    );
  }
  ...
  const iconColor = (scrolled || !isHomePage) ? 'text-gray-500' : 'text-white'; // Color for icons
          ...
          {navLinks}
           ...
           <div className="relative">
              <div className="flex items-center space-x-1 cursor-pointer">
                 ...
                 >
                    {/* --- UPDATED: Using indianCities list --- */}
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-white/95 backdrop-blur-sm absolute top-20 left-0 w-full shadow-lg">
          <div className="flex flex-col items-center space-y-6 py-8">
            ...
            <Link href="#" className="text-gray-700 hover:text-pink-500 text-lg" onClick={() => setIsMenuOpen(false)}>About</Link>
            <Link href="#" className="text-gray-700 hover:text-pink-500 text-lg" onClick={() => setIsMenuOpen(false)}>Contact</Link>
            ...
            <Link href="#" className="py-2 px-6 bg-pink-600 text-white rounded-full hover:bg-pink-700" onClick={() => setIsMenuOpen(false)}>Login</Link>
          </div>
        </div>
      )}
    </nav>
